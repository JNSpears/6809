...
*                                                *
**************************************************
ROMS    EQU   $2000          ; JNS was $d000, was $E000
RAMS    EQU   $1000          ; Start of RAM ; JNS - Can not be 0000
EM      EQU   $A000          ; End of RAM + 1 ; JNS - Below MPX/9
NSCR    EQU   $0000          ; Number of 1024 byte screens
KBBUF   EQU   $0100          ; Data bytes per disk buffer
US      EQU   $0040          ; User variable space
RTS     EQU   $00A0          ; Return stack and terminal buffer
CO      EQU   KBBUF+4        ; Data bytes per buffer plus 4
NBUF    EQU   4              ; Number of buffers
...
ACIAC   EQU   $F7FE           ; ACIA Control -- JNS works for Percom SBC/9
ACIAD   EQU   ACIAC+1         ; ACIA Data -- JNS works for Percom SBC/9
	endc
...
*                                                *
**************************************************
        ORG   ROMS		; JNS was $d000, was $E000
ORIG
	ifndef PSYMON
		LDA   #$03           ; Configure the ACIA
		STA   ACIAC
		LDA   #$16
		STA   ACIAC
	endc
        ...
        ifndef PSYMON
                ...
                ; S is ok just use the MPX9 stack.
                LEAU    -100,S  ; take space below System stack for User stack
        endc
        
        ; Set up is done, now do FORTH
        LDY   #ENTRY
        LDX   ,Y++           
        JMP   [,X++]
...
COLD    FDB   DOCOL
	...
	FDB   DECA             ; JNS - Force it to decimal
...
PKEY1   
	ifdef	PSYMON
		...
		LDB   #StatFn	; SET UP FOR Status Check
		JSR   [ReqIOv]	; Get control status of the ACIA
		ASRA		; Check if buffer is empty
		BCC   PKEY1
		LDB   #ReadFn	; SET UP FOR READ
		JSR   [ReqIOv]	; READ A CHARACTER
		ANDA  #$7F	; REMOVE PARITY
		TFR   A,B	; Put in the right place.
	else
...
PEMIT1  
	ifdef	PSYMON
		...
		JSR   [ReqIOv]	; OUTPUT THE CHARACTER
	else
...
EEMO1   FDB   DUP
 ...
 ENDC
        ...
        ;
        ; addition psymon commands.
        ;
        
        ifdef   PSYMON
****************************************************************************************************
*
* JNS+ additional commands for Percom SBC/9, PsyMon & MPX/9
*
****************************************************************************************************
...
PPSYMON FDB   *+2
        ...
        ;
        ; dummy command 'XXXX' does nothing.
        ;
        WORDM   4,'XXXX'
XXXX    FDB   DOCOL
        FDB   SEMIS
...
****************************************************************************************************
 WORDM 5,'MLOAD' ; ( cmd-addr MLOAD;  cmd@ ... )
        ...
        FDB FROMR,TIB,STORE
        FDB FROMR,INN,STORE
        ; FDB QUIT ;JNS i'm not sure why i commented this out?
        FDB SEMIS
...
;
; JNS- end of additional commands for Percom SBC/9, PsyMon & MPX/9
;
